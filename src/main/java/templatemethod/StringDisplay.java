...
public class StringDisplay extends AbstractDisplay { //StringDisplay도 AbstrctDisplay의 하위 클래스
    ...
    public StringDisplay(String string) {           //생성자에서 전달된 문자열 string을 필드에 기억
        this.string = string;
        this.width = string.getBytes().length;      //그리고 바이트 단위의 길이도 필드에 기억해 두고 나중에 사용한다.
    }

    /**
     * 오버라이드해서 정의한 open 메소드
     * 이 클래스의 메소드는 printLine에서 선을 그리고 있다.
     */
    @Override
    public void open() {
       printLine();
    ...
    /**
     * close 메소드는
     * open처럼 printLine 메소드에서 선을 그리고 있다.
     */
    @Override
    public void close() {
        printLine();
    ...
    /**
     * open과 close에서 호출된 printLine 메소드이다.
     * private이기 때문에 이 클래스 안에서만 사용된다.
     */
    private void printLine() {
        System.out.print("+");            //테두리의 모서리를 표현하는 "+" 마크를 표시
        for (int i = 0; i < width;  i++) {  //width개의 "-"을 표시하고 테두리 선으로 이용한다.
            System.out.print("-");
        }
